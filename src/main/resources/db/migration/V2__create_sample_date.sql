...
WHERE NOT EXISTS (SELECT 1 FROM category c WHERE c.category_id = t.category_id and c.category_name = t.category_name);
...
INSERT INTO book (author, is_available, title)
SELECT author, is_available, title
FROM (SELECT '권태영' as author, 1 as is_available, '너에게 해주지 못한 말들' as title) t
WHERE NOT EXISTS (SELECT 1 FROM book b WHERE b.author= t.author and b.title = t.title);
